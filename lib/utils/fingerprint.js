...
export const getOrCreateFingerprint = () => {
  ...
  if (!fingerprintData) {
    ...
    Cookies.set(fingerprintKey, JSON.stringify(newFingerprint), { expires: 7, sameSite: 'Lax' });
...
export const updateFingerprintStatus = (status) => {
  ...
  if (fingerprintData) {
    try {
      ...
      Cookies.set(fingerprintKey, JSON.stringify(fingerprintData), { expires: 7, sameSite: 'Lax' });
    } catch (error) {
