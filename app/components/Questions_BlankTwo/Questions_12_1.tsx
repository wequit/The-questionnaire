...
interface Question_Twelve_One_Props {
  questions: Question[];
}
export default function Question_Twelve_One({ questions }: Question_Twelve_One_Props) {
  const { selectedOptions, otherText, handleOptionChange, handleOtherTextChange } = useQuestionCheckboxes({
    localStorageKey: "Question_12_1",
    localStorageOtherKey: "Question_12_1_other",
  });

  const question = questions.find((q) => q.id === 14);
