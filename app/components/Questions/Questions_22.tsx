...
interface Question_TwentyTwo_Props {
  questions: Question[];
}

export default function Question_TwentyTwo({ questions }: Question_TwentyTwo_Props) {
  const [windowWidth, setWindowWidth] = useState(window.innerWidth);
  const { language } = useLanguage();
  const { setValidError, getValidError } = useAnswerContext();
  const question = questions.find((q) => q.id === 22);
