...
export default function Question_Eight() {
  const [selectedOption, setSelectedOption] = useState<string | null>(null);
  const [isSubmitted, setIsSubmitted] = useState(false);

  useEffect(() => {
    const savedOption = localStorage.getItem("Question_8");
    if (savedOption) {
  ...
  const handleOptionChange = (option: string) => {
    ...
    localStorage.setItem("Question_8", option);
    setIsSubmitted(false);
  };

  return (
    <section className="p-6">
      {Questions.QuestionEight.map((questionData, index) => (
        <div key={index} className="mb-6">
          <h2 className="text-lg font-bold text-gray-900 mb-4">
            {questionData.questionEight}
          </h2>
          <div className="text-gray-700 mb-6 mt-8">
            {questionData.optionsEight.map((option, i) => (
