...
interface Question_Twenty_Props {
  questions: Question[];
}

export default function Question_Twenty({ questions }: Question_Twenty_Props) {
  const [windowWidth, setWindowWidth] = useState(window.innerWidth);
  const { language } = useLanguage();
  const { setValidError, getValidError } = useAnswerContext();
  const question = questions.find((q) => q.id === 20);
